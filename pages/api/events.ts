...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    const matches = rules.length === 0 || rules.some((rule: any) => {
      if (!rule.enabled) return false;

      if (rule.deviceId && rule.deviceId !== event.deviceId) return false;
  ...
  if (req.method === "GET") {
    ...
    return res.status(200).json(events.map(e => JSON.parse(e)));
  }
